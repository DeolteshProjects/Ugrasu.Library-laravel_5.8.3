...
class PrintLibraryReport extends Model
{
    ...
    public function printOnlyOneSpecial($year,$specialitycode)
    {
        ...
        for ($i = 0; $i < $AmountOfLibraryReport; $i++) {
            ...
            if (((isset($LibraryReports[$i]->literature['AmountOftBookLiterature']))) AND ((isset($LibraryReports[$i]->literature['AmountOfeBookLiterature'])))) {
                (new PrintLibraryReport())->setTwoViewBooks($template, $LibraryReports[$i]->literature, $row);
            } else if ((!isset($LibraryReports[$i]->literature['AmountOftBookLiterature']))) {
                //Если есть только печатная литература
                (new PrintLibraryReport())->setOnlyOneViewBooks($template, 1, $LibraryReports[$i]->literature['eBook'], $LibraryReports[$i]->literature['AmountOfeBookLiterature'], $row);
            } else if ((!isset($LibraryReports[$i]->literature['AmountOfeBookLiterature']))) {
                //Если есть только печатная литература
                (new PrintLibraryReport())->setOnlyOneViewBooks($template, 0, $LibraryReports[$i]->literature['tBook'], $LibraryReports[$i]->literature['AmountOftBookLiterature'], $row);
            }
        }
    ...
    public function printOnlyOneDisc($year,$specialitycode, $disciplinecode)
    {
