@extends('layouts.app')
@section('content')CompilingLibraryReport.blade.php
    <div class="col-lg-12">
        <div class="col-12">
            <div class="card">
                <div class="card-body">
                                    ...
                                    <div class="dropdown-menu">
                                        ...
                                        <div class="alert-danger">
                                            <button class="dropdown-item btn-outline-danger"
                                                    onclick="CleanCreatingLibraryReport()">Очистить литературу
                                            </button>
                                            <button class="dropdown-item btn-outline-red"
                                                    onclick="DeleteCreatingLibraryReport()">Удалить справку
                                            </button>
                                        </div>
                                    </div>
                                </div>
    ...
    <script>
        ...
        function FinishAndSaveLibraryReport() {
            ...
            $.ajax({
                url: '{{ route( 'LibraryReportDisc.saveCreatingLibraryReportInDB') }}',
                type: 'POST',
                async: true,
                headers: {
                ...
                success: function (data) {
                    Notification('Библиографическая справка успешно создана', 'Вы можете наблюдать за ней на страницу <a href="{{ route('home') }}">"Составленные"</a>', 'success');
        ...
        function CleanCreatingLibraryReport() {
            $.ajax({
                url: '{{ route( 'LibraryReportDisc.cleanCreatingLocalLibraryReport') }}',
                type: 'POST',
                async: true,
                headers: {
                    'X-CSRF-Token': $('meta[name="csrf-token"]').attr('content')
        ...
        function deleteBookFromLibraryReport(id, view) {
            $.ajax({
                url: '{{ route( 'LibraryReportDisc.deleteBookInCreatingLocalLibraryReport') }}',
                type: 'POST',
                data: {
                    Id: id,
                    View: view
